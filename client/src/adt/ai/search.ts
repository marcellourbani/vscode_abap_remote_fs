import { CancellationToken, LanguageModelPromptTsxPart, LanguageModelTextPart, LanguageModelTool, LanguageModelToolInvocationOptions, LanguageModelToolInvocationPrepareOptions, LanguageModelToolResult, PreparedToolInvocation, ProviderResult, Uri } from "vscode"
...
export class SearchTool implements LanguageModelTool<SearchInput> {
    async invoke(options: LanguageModelToolInvocationOptions<SearchInput>, token: CancellationToken): Promise<LanguageModelToolResult> {
        ...
        for (const finding of raw.slice(0, 10)) {
            ...
            if (node) {
                ...
                findings.push({ name: finding["adtcore:name"], type: finding["adtcore:type"], uri: u.toString() })
            }
            if (findings.length >= 10) break
        }
        const content = new LanguageModelTextPart(JSON.stringify(findings))
        return new LanguageModelToolResult([content])

    }
    prepareInvocation?(options: LanguageModelToolInvocationPrepareOptions<SearchInput>, token: CancellationToken): ProviderResult<PreparedToolInvocation> {
        const uri = Uri.parse(options.input.url)
        const client = getClient(uri.authority)
        if (client)
            return { invocationMessage: `Searching abap object ${options.input.type || ""} ${options.input.name} ${uri.authority}` }
