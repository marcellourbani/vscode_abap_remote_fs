import { Uri, tests, TestRunProfileKind, TestRunRequest, CancellationToken, TestController, TestItem, TestMessage, MarkdownString, commands, TestItemCollection } from "vscode"
...
const classId = (cl: UnitTestClass) => `${cl["adtcore:type"]} ${cl["adtcore:name"]}`
const methodId = (meth: UnitTestMethod) => `${meth["adtcore:name"]}`
...
const alertIcon = (alert: UnitTestAlert) => {
  switch (alert.kind) {
    ...
    case UnitTestAlertKind.warning:
      return "testing-passed-icon"
  }
}


const convertAlert = (alert: UnitTestAlert): TestMessage => new TestMessage(new MarkdownString(`${alertIcon(alert)} ${alert.details.join("<br>")}`, true))
...
const runHandler = async (
  ...
  const connId = request.include?.[0].uri?.authority
  if (!connId) throw new Error("No valid rest in request")
  const run = UnitTestRunner.get(connId).controller.createTestRun(request)
  try {
    const excluded = (i: TestItem) => request.exclude?.find(ii => i === ii)
  } finally {
    run.end()
  }
  log(JSON.stringify(request, undefined, 1))
}
...
const dummyRunHandler = async (ctrl: TestController, root: TestItemCollection, classes: UnitTestClass[]) => {
  ...
  for (const clas of classes) {
    ...
    for (const meth of clas.testmethods) {
      ...
      if (outcome.passed)
        run.passed(methTI, meth.executionTime)
      else
        run.failed(methTI, outcome.messages, meth.executionTime)
    }
  }
  run.end()
}
...
const getOrCreateObjectItem = async (c: TestController, object: AbapObject, classes: UnitTestClass[], uri: Uri) => {
  const item = c.items.get(object.key) || c.createTestItem(object.key, object.key, uri)
  const children: TestItem[] = []
...
export class UnitTestRunner {
  ...
  private constructor(readonly connId: string) {
    ...
    this.controller.createRunProfile(`ABAP ${connId}`, TestRunProfileKind.Run, runHandler, true)
  }
