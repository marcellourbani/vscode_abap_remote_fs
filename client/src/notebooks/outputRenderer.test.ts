vi.mock("vscode", () => {
  const mockCellOutputItem = {
    text: vi.fn(function (value: string, mime: string) {
      return { data: Buffer.from(value), mime }
    }),
    json: vi.fn(function (value: unknown) {
      return {
        data: Buffer.from(JSON.stringify(value)),
        mime: "application/json"
      }
    }),
    error: vi.fn(function (err: any) {
      return {
  ...
  const mockNotebookCellOutput = vi.fn().mockImplementation(function (items: any[]) {
    return { items }
  })
  return {
...
describe("renderSqlOutput", () => {
  ...
  test("builds HTML table for rows", () => {
    ...
    renderSqlOutput(result)
    const { NotebookCellOutputItem } = __$mock_vscode
    const call = vi.mocked(NotebookCellOutputItem.text).mock.calls[0]
  ...
  test("uses row keys as column names when columns array is empty", () => {
    ...
    renderSqlOutput(result)
    const { NotebookCellOutputItem } = __$mock_vscode
    const html = vi.mocked(NotebookCellOutputItem.text).mock.calls[0][0]
...
describe("renderJsOutput", () => {
  ...
  test("renders only logs when result is undefined but logs exist", () => {
    ...
    expect(output).toBeDefined()
    const { NotebookCellOutputItem } = __$mock_vscode
    const calls = vi.mocked(NotebookCellOutputItem.text).mock.calls
