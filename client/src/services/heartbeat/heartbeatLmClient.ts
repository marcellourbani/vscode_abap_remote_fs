...
function buildHeartbeatPrompt(config: HeartbeatConfig): string {
  ...
  if (hasAnyDueTasks) {
    ...
    lines.push('2. Call abapfs_manage_heartbeat with action "remove_task" to delete it')
    lines.push("")
    lines.push("### For Monitoring Tasks with SQL:")
    lines.push("1. Call abapfs_run_sql_query with the provided sampleQuery")
    lines.push("2. Follow the checkInstructions to interpret results")
    lines.push('3. Call abapfs_manage_heartbeat "update_task" to save your findings in lastResult')
    lines.push("4. If new issues found AND not in cooldown → include alert in your response")
    lines.push("")
    lines.push("### For Tasks using abapfs_analyze_dumps:")
    lines.push('1. Call abapfs_analyze_dumps with action "list_dumps"')
