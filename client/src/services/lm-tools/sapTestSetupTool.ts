...
export class SapTestSetupTool implements vscode.LanguageModelTool<Record<string, never>> {
  ...
  async invoke(
    ...
    const result = [
      ...
      '4. User can configure testing-agent models by running command "ABAP FS: Set Models for Subagents". Alternatively, you can offer to configure them with the abapfs_manage_subagents tool, but you must get user approval before changing model settings.',
      "5. Start the workflow with /sap-testing.",
      "",
      'For authoritative details, call the ABAP FS documentation tool "abapfs_search_documentation" with action "search_documentation" and searchQuery "SAP Testing" before answering follow-up questions.',
...
export function registerSapTestSetupTool(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    registerToolWithRegistry("abapfs_setup_sap_testing", new SapTestSetupTool())
  )
}
