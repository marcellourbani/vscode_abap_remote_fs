;(() => {
  ...
  const doSearch = () => {
    ...
    if (!/[\*\?]/.test(term)) term = `${term.toUpperCase()}*`
    else term = term.toUpperCase()
  ...
  const operatorOptions = type => {
    const common = [
      ...
      { v: "!=", t: "!=" },
      { v: "like", t: "LIKE" },
      { v: "not like", t: "NOT LIKE" },
      { v: "in", t: "IN" },
      { v: "not in", t: "NOT IN" },
      { v: "is initial", t: "IS INITIAL" },
      { v: "is not initial", t: "IS NOT INITIAL" }
    ]
    const numeric = [
      { v: ">", t: ">" },
      { v: ">=", t: ">=" },
    ...
    if (["I", "b", "8", "s", "P", "/", "a", "e", "F", "N", "%"].includes(type))
  ...
  const renderCriteria = () => {
    ...
    list.forEach(col => {
      ...
      tdField.innerHTML = `<div><strong>${col.description || col.name}</strong><div class="adb-tech">${col.name} · ${col.type}</div></div>`
      tr.appendChild(tdField)
      const tdOp = document.createElement("td")
      const sel = document.createElement("select")
      operatorOptions(col.type).forEach(op => {
      ...
      chkShow.title = "Show column"
      chkShow.className = "adb-show"
      ...
      chkShow.addEventListener("change", () => {
        showPref.set(col.name, chkShow.checked)
      })
      tdShow.appendChild(chkShow)
    ...
    const setAll = val => {
      ...
      criteriaPanel.querySelectorAll("input.adb-show").forEach(ch => {
        ch.checked = val
      })
    }
  ...
  const buildWhere = () => {
    ...
    rows.forEach(tr => {
      ...
      if (op === "like" || op === "not like") {
        ...
        if (parts.length > 1) {
          const orClauses = parts.map(
            part => `${field} ${op.toUpperCase()} ${sqlQuote(likePattern(part.trim()))}`
          )
  ...
  if (fieldsPrev)
    fieldsPrev.addEventListener("click", () => {
      fieldPage = Math.max(1, fieldPage - 1)
      renderCriteria()
    })
  if (fieldsNext)
    fieldsNext.addEventListener("click", () => {
  ...
  if (toggleFieldsBtn)
    toggleFieldsBtn.onclick = () => {
      ...
      if (visible) {
        criteriaPanel.style.display = "none"
        if (h) h.style.display = "none"
        toggleFieldsBtn.textContent = "Show Fields"
      } else {
        if (h) h.style.display = ""
        criteriaPanel.style.display = ""
        toggleFieldsBtn.textContent = "Hide Fields"
      }
    }
  ...
  if (sqlModal)
    sqlModal.addEventListener("click", e => {
      if (e.target === sqlModal) sqlModal.style.display = "none"
    })
  if (sqlCopy)
  ...
  const renderResult = result => {
    ...
    if (rawCols.length === 0) {
      document.getElementById("result-table").innerHTML =
        '<p style="padding: 12px;">No data found.</p>'
      updateActionButtonsVisibility()
      return
    }
    ...
    if (!table) {
      table = new Tabulator("#result-table", {
        ...
        rowContextMenu: [{ label: "Copy Row", action: (e, row) => row.copyToClipboard() }],
        columnHeaderMenu: [
  ...
  exportCsvBtn.onclick = async () => {
    ...
    const columns = table
      .getColumns()
      .map(c => ({ title: c.getDefinition().title, field: c.getField() }))
    const rows = table.getData()
    vscode.postMessage({
      command: "exportCSV",
      columns,
      rows,
      defaultName: (currentEntity && currentEntity.name) || "data"
    })
  }
    ...
    switch (msg.command) {
      ...
      case "fields":
        ...
        if (currentEntity?.name)
          vscode.postMessage({ command: "getShowPrefs", table: currentEntity.name })
        renderCriteria()
        break
      case "showPrefs": {
        ...
        if (
          currentEntity?.name &&
          table &&
          currentEntity.name.toUpperCase() === String(table).toUpperCase()
        ) {
          showPref = new Map()
      ...
      case "error":
        ...
        document.getElementById("result-table").innerHTML =
          `<p style="color: red; padding: 12px;">${msg.data}</p>`
        clearBusy()
        break
    }
  })
  ...
  if (copyRowsBtn)
    copyRowsBtn.onclick = async () => {
      ...
      const cols = table
        ...
        .filter(Boolean)
      const csv = [cols.join("\t")]
        .concat(rows.map(r => cols.map(f => String(r[f] ?? "")).join("\t")))
        .join("\n")
      try {
