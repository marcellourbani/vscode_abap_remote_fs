...
6. Call `check_test_data` for the program + exact connectionId. If it reports any FAIL for a selected case, STOP that case and follow `prepare-data` to resolve it. Do not start Playwright hoping runtime resolution will work.
...
1. **Read the `playwright_test` tool's output** — it summarizes pass/fail per test and includes the first few lines of any error.
2. **Read `tests/<PROGRAM>/test-results/<connectionId>/<TC-ID>/manifest.json`** — last recorded step shows where progress stopped.
3. **Look at the last screenshot** in `tests/<PROGRAM>/test-results/<connectionId>/<TC-ID>/step-*.png`.
4. **Categorize**:
...
- Traces are captured on failure (`trace: "retain-on-failure"`) under the test's `test-results/<connectionId>/<TC-ID>/` folder — open with the Playwright VS Code extension if the user has it installed, or `npx playwright show-trace <path>` if they have Node available (optional; not required for anything else in this project).
