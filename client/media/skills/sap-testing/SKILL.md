...
| ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| The **test folder** (`sapTesting.testFolder` setting — ask the user to run "SAP Testing: Set Test Folder" if unset) | `tests/<PROGRAM>/test-cases/*.md`, `test-scripts/*.spec.ts`, `test-results/**`, one managed `tsconfig.json` | Yes — this is a normal, git-trackable project folder. Read/write it with your normal file tools.                                   |
...
| Tool                     | Use for                                                                                                                                                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `get_sap_webgui_url`     | Given an abapfs `connectionId`, reads `abapfs.remote` and returns the full SAP WebGUI URL. **Call this whenever you need to open a browser for live exploration or to pass the URL to `playwright_test`.**                     |
...
5. **No login logic in a spec, ever.** Never write credentials, a logon step, or an auth workaround into a `.spec.ts`, `.md`, or `.data.md`. If a browser session isn't authenticated, ask the user to log in to SAP in that browser window and continue once they confirm.
