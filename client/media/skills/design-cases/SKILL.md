...
2. Count **each individual IF** in `_findings.md` — not "branch groups". Every single `IF` condition is a distinct path and needs its own TC unless it is purely internal (loop counter, length check with no user-visible outcome). If you merged two IFs into one TC, split them.
...
verification: sql | manual | mixed | none # mandatory — HOW this case proves the object did its job (see "Post-test verification is the rule" below). none is only valid for a case that persists/emits nothing (pure error/abort case).
outputs: [alv | excel | email | al11 | table | idoc | job | spool]
messagesExpected: [] # MANDATORY: every message this case triggers, as "class-num" tuples (e.g. ["ZTESTMSG-001", "ZTESTMSG-004"]). ANY case whose Expected Result names a message text MUST list its class-num here. Empty ONLY when the case triggers no message at all (rare — typically pure background-artifact or silent happy path). "class-num" strings come straight from `_findings.md`'s MESSAGE table.
messagesForbidden: []
tags: []
...
- **`sql`** — every check is a DB read the model can run in Phase 7. This covers most effects, because they land in queryable tables: application tables (`ZMMT_...`), IDocs (`EDIDC`/`EDIDS`/`EDID4`), background jobs (`TBTCO`/`TBTCP`), change docs (`CDHDR`/`CDPOS`), spool (`TSP01`). Prefer `sql` whenever the effect is queryable.
...
- **`mixed`** — some checks `sql`, some `manual`.
- **`none`** — VALID ONLY for a case that persists/emits nothing: a pure error/validation case where the object aborts before writing anything (the error MESSAGE itself is the entire outcome). If you set `none`, it must be obvious from the case (an error case) or stated in Notes — the reviewer challenges every `none` against `_units.md`.
...
**Authorization cases have a manual user-provisioning prereq.** An `authorization` TC comparing "with role X" vs "without role X" needs a real SAP user for BOTH sides — the current agent user for the positive case, and a separately-provisioned unauthorized user (e.g. `ZTESTNOAUTH`) for the negative case. This user must exist on the target `connectionId` before Phase 7 can run the negative case; it is NOT something `prepare-data` can create because user provisioning goes through SU01 / the customer's identity system, not through any test-writable path. In the TC's `## Preconditions` section, name the exact unauthorized user (or role gap) required, and note in `## Notes for automation` that provisioning is a manual prereq. The `.data.md` may pin it via `source: static` (`staticValue: "ZTESTNOAUTH"`) or `source: user` so a run against a different landscape can substitute a different unauthorized user without editing the spec.
