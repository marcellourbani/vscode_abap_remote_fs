...
;(() => {
  ...
  function getBorderColorForType(type) {
    ...
    return baseColor.replace(
      /(\d+)%\)$/,
      (match, lightness) => `${Math.max(0, parseInt(lightness) - 15)}%)`
    )
  }
  ...
  let createCytoscapeGraph = data => {
    ...
    if (!data || !data.nodes || data.nodes.length === 0) {
      graphContainer.innerHTML =
        '<div style="padding: 20px; text-align: center;">No dependencies found</div>'
      hideBusy()
      return
    }
  ...
  const applyFilters = async () => {
    ...
    const selectedTypes = Array.from(document.querySelectorAll(".type-checkbox:checked")).map(
      cb => cb.value
    )
    const selectedUsageTypes = Array.from(document.querySelectorAll(".usage-checkbox:checked")).map(
      cb => cb.value
    )
    ...
    if (filteredNodes.length > 1000) {
      const proceed = confirm(
        `About to render ${filteredNodes.length} nodes. This may take a while. Continue?`
      )
      if (!proceed) {
        hideBusy()
        return
  ...
  const buildUsageFilters = types => {
    ...
    if (types.length === 0) {
      usageFiltersContainer.innerHTML =
        '<div style="color: #888; font-size: 11px;">No usage types</div>'
      return
    }
