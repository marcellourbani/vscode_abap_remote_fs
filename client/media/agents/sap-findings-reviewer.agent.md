...
4. **Branch & auth completeness.** Compare the count and spread of `IF/ELSEIF/CASE/WHEN/AUTHORITY-CHECK` in the source to `_findings.md`. Flag whole sections of a method that produced no branch rows.
...
9. **Target-minimum honesty.** Is the preliminary target minimum a real sum of the (corrected) candidate rows, or a round low number? Given the messages + branches + auth + behavioural rules you found, is it plausibly complete, or does it obviously undercount (missing the value-default cases, an error path, per-field boundaries)? If low relative to the real surface, FAIL with the gap.
