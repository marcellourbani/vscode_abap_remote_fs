...
8. **Cross-check post-test verification against `_units.md` — using these exact criteria (do NOT demand `mixed` reflexively).** Judge each case's `verification` value against what the case's path actually does, per `_units.md`'s "Effective outputs":
   ...
   - **Every `verification: sql|mixed` case requires complete dual proof:** authoritative SQL in Phase 7 plus meaningful SE16N screenshots for EVERY SQL-verified business table/effect users can inspect. Verify `se16nTables` is complete against all post-test rows and `_units.md` outputs. Example: a PO case verifying EKKO and EKPO must list both; `[EKKO]` is a FAIL, not acceptable minimum compliance. If a SQL target cannot be displayed in SE16N, require an explicit note and report the mandatory gate as a blocker rather than silently omitting it.
