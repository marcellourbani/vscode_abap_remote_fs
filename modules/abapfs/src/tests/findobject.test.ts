...
test(
  ...
  runTest(async root => {
    ...
    if (!isAbapStat(abapgit?.file)) throw new Error("Abap Object expected")
...
test(
  ...
  runTest(async root => {
    ...
    if (!isAbapFile(abapgit?.file)) throw new Error("Leaf node expected for main program include")
...
test(
  ...
  runTest(async root => {
    ...
    if (!isAbapStat(tabledesc?.file)) throw new Error("Abap Object expected")
    ...
    if (!isAbapFile(tabledesc?.file)) throw new Error("Leaf node expected for main program include")
...
test(
  ...
  runTest(async root => {
    ...
    if (!isAbapFile(tabledesc?.file)) throw new Error("Abap Object expected")
    expect(tabledesc?.file.object.name).toBe("CL_ABAP_TABLEDESCR.main")
    expect(tabledesc?.path).toBe(
      "/System Library/BASIS/SABP_MAIN/SABP_RTTI/Source Code Library/Classes/CL_ABAP_TABLEDESCR/CL_ABAP_TABLEDESCR.clas.abap"
    )
